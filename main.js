...
function addRow(sectionId) {
    ...
    <div class="row-input" id="row-${globalRowId}" style="display:flex; flex-wrap:wrap; gap:8px; align-items:flex-end; padding:12px 0; border-bottom:1px dashed #eee;">
        ...
        <div class="input-group" style="width: 230px; position:relative; flex:none;">
             <span class="input-label" style="color:#1677ff">发票 (上传或从票夹选)</span>
             <div style="display:flex; gap:4px;">
                 <input type="file" name="invoice_${globalRowId}[]" multiple accept="image/*,.pdf" onchange="scanInvoiceQR(this, ${globalRowId})" style="font-size:11px; height:32px; padding-top:3px; flex:1;">
                 
                 <button type="button" class="btn btn-ghost btn-sm" onclick="selectFromWallet(this, ${globalRowId})" title="从票夹选择" style="height:32px; padding:0 8px; border:1px solid #ddd;">
                    <i class="ri-wallet-3-line" style="color:#1890ff;"></i>
                 </button>
             </div>
             <input type="hidden" name="items[${globalRowId}][wallet_ids]" class="wallet-ids-input">
             <div id="wallet-badge-${globalRowId}" style="font-size:10px; color:#1890ff; position:absolute; top:-18px; right:0; display:none; background:#e6f7ff; padding:0 4px; border-radius:4px;"></div>
